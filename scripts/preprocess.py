...
def main() -> None:
    ...
    track_counts = raw.groupby(["source", "agent_id"]).size().rename("raw_observations").reset_index()
    generated = windows.groupby(["source", "agent_id"]).size().rename("generated_windows").reset_index()
    track_counts = track_counts.merge(generated, how="left", on=["source", "agent_id"])
    track_counts["generated_windows"] = track_counts["generated_windows"].fillna(0).astype(int)
    span = 1 + (cfg.obs_len - 1) * (cfg.max_obs_skip if cfg.irregular_obs else 1)
    track_counts["eligible_starts"] = np.maximum(
        0, 1 + (track_counts["raw_observations"] - span - cfg.pred_len) // cfg.stride
    )
    track_counts["requested_windows"] = track_counts["eligible_starts"] * (
        cfg.irregular_samples if cfg.irregular_obs else 1
    )
    track_counts.to_csv(out_dir / "track_window_counts.csv", index=False)
    ...
    summary = {
        ...
        "stride": args.stride,
        "fps": args.fps,
        "timestamp_precision": "float64 before interval subtraction",
        "fps_provenance": "user-supplied fallback; verify per-source frame numbering and time units",
        "raw_rows_definition": "unique retained source-agent-timestamp observations, not windows",
        "window_count_definition": "eligible starts times irregular_samples; overlapping windows reuse observations",
        "max_track_gap": args.max_track_gap,
        "max_link_distance": args.max_link_distance,
        "irregular_obs": args.irregular_obs,
        "max_obs_skip": args.max_obs_skip,
